...
const weatherToMood = (weatherData) => {
    ...
    if (weatherCondition.includes("Clouds") || weatherCondition.includes("Rain") || weatherCondition.includes("Snow")) {
        mood = "sad";
...
router.get('/', async (req, res) => {
    ...
    try {
        const weatherResponse = await fetch(`https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lng}&appid=${openWeatherMapApiKey}`);
        const weatherData = await weatherResponse.json();
        console.log(weatherData);
        const mood = weatherToMood(weatherData);
        console.log(mood);
        res.redirect(`http://localhost:5010/api/spotify/login?mood=${mood}`);
    } catch (error) {
